...
public class GameController : MonoBehaviour
{
    ...
    // Used for viewing previous move
    public Texture2D PreviousMove;
    ...
    private void OnGUI()
    {
        ...
        {
            ScreenShot = false;
            PreviousMove = new Texture2D(Screen.width, Screen.height);
            PreviousMove.ReadPixels(new Rect(0, 0, Screen.width, Screen.height), 0, 0);
            PreviousMove.Apply();
            ScreenShotDone = true;
            SetPiece(_moveAI, _pieceLocations[_moveAI].transform.position); // Sets the calculated move on the board
        }
        ...
        {
            string iq = (GameOptions.Instance.SMART) ? "SMART" : "DUMB";
            Vector2 size = GUI.skin.label.CalcSize(new GUIContent("BEFORE AI"));
            Rect image = new Rect(Screen.width - (Screen.width / 5 + 20), Screen.height - (Screen.height / 5 + 20), Screen.width / 5, Screen.height / 5);
            GUI.Label(new Rect(image.x + (image.width / 2 - size.x / 2), image.y - size.y, size.x, size.y), "BEFORE AI");
            GUI.DrawTexture(image, PreviousMove, ScaleMode.ScaleToFit);
        }
    }
